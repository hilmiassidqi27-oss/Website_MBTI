...
import { getStoredAccessToken, getStoredSpreadsheetId, appendSubmissionToGoogleSheet } from './lib/googleSheets';
...
export default function App() {
  ...
  // Save submission to Firestore and optionally sync to Google Sheets
  const saveSubmission = async (newSub: AssessmentSubmission) => {
    ...
    if (sheetId && token) {
      appendSubmissionToGoogleSheet(token, sheetId, newSub, submissions.length + 1).catch((err) => {
        console.warn('Silent Google Sheet auto-append note:', err);
      });
    }
  };
