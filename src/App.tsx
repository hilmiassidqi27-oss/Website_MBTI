...
export default function App() {
  ...
  useEffect(() => {
    // 1. Initial manual fetch as fallback/verification
    getAllSubmissionsFromFirestore()
      .then((firestoreData) => {
      ...
      .catch((err) => {
        console.warn('Initial direct Firestore fetch warning:', err);
      });
    ...
    const unsubSubmissions = subscribeToSubmissions(
      (firestoreData) => {
        ...
        // Check if there are any local-only submissions that haven't reached Firestore
        const cachedStr = localStorage.getItem('dpp_mbti_submissions');
        if (cachedStr) {
          try {
            const cached: AssessmentSubmission[] = JSON.parse(cachedStr);
            const remoteIds = new Set(firestoreData.map((s) => s.id));
            const missing = cached.filter((s) => !remoteIds.has(s.id));
            if (missing.length > 0) {
              missing.forEach((m) => {
                saveSubmissionToFirestore(m).catch(() => {});
              });
            }
          } catch (_) {}
        }
  ...
  const handleRefreshCloud = async () => {
    try {
      const data = await getAllSubmissionsFromFirestore();
      setSubmissions(data);
      localStorage.setItem('dpp_mbti_submissions', JSON.stringify(data));
      return data;
    } catch (e) {
      console.error('Failed to manually refresh from cloud:', e);
      throw e;
    }
  };
